	...
	return previewers.new({
		...
		preview_fn = function(self, entry, status)
			local function get_file_display_text(bufname)
				local bufpath = hbac_utils.format_filepath(bufname)
			...
			local stored_pins = entry.value.stored_pins
			local lines = {}

			local timestamp = entry.value.timestamp
			...
			for _, pin in pairs(stored_pins) do
				...
				local devicon, hl = hbac_utils.get_devicon(full_path)
